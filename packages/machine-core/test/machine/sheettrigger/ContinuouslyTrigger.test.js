...
describe('ContinuouslyTrigger', () => {
	...
	// });
	it.skip('should process sheet on each machine step', async () => {
	...
	it('should not increase sheet steps on "repeat until..." steps outside sheet steps', async () => {
		const { machine, s1 } = setup();
		s1.trigger.update({ repeat: 'endless' });
		createCellAt('A1', { formula: 'A1+1' }, s1.sheet);
		createCellAt('A2', { formula: 'if(mod(A1,3)=0,return(),false)' }, s1.sheet);
	...
	it.skip('should run "repeat until..." steps outside sheet steps', async () => {
		expect(false).toBe(true);
		// same setup as before and then start machine and stop it after 200 ms => must have complete processed
		// 
	});

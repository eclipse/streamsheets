 ...
 ********************************************************************************/
 ...
 module.exports = class SetCellsCommand extends AbstractItemCommand {
	 static createFromObject(data = {}, { graph }) {
		 ...
		 return item ? new SetCellsCommand(item, data.cells, data.execute).initWithObject(data) : undefined;
	 }
	 ...
	 get streamsheetId() {
		 return this.sheet ? getStreamSheetId(this.sheet) : undefined;
	 }
 
	 combine(cmd) {
		 this._execute = this._execute || cmd._execute;
		 // currently don't care about same references in descriptors
		 cmd._cellDescriptors.forEach((descr) => this._cellDescriptors.push(descr));
		 return this;
	 }
	 ...
	 toObject() {
		 ...
		 data.streamsheetId = this.streamsheetId;
		 return data;
	 }
	 ...
	 execute() {
		 ...
		 dataProvider.enumerate((column, row, cell) => {
			 ...
			 range.shiftToSheet();
			 this._tmpCellDescriptors[range.toString()] = cellDescriptor(range.toString(), cell);
		 });
	 ...
	 setCells(cellDescriptors = []) {
		 ...
		 cellDescriptors.forEach((cellData) => {
			 ...
			 if (res != null) {
				 const pos = new Point(res.column - this._graphItem.getColumns().getInitialSection(), res.row);
		 ...
		 if (response && !this._oldCellDescriptors) {
			 ...
			 Object.keys(sheetcells).forEach((key) => {
				 ...
				 if (!oldCellDescr || !areEqual(oldCellDescr, newCellDescr)) {
					 this._oldCellDescriptors.push(oldCellDescr || cellDescriptor(key));
				 }
			 });
		 } else {
			 // if (error)

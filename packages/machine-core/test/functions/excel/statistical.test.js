...
describe('statistical functions', () => {
	...
	describe('stdev', () => {
		it('should return standard derivation of given samples', () => {
			...
			expect(createTerm('stdev(A2:A11)', sheet).value.toFixed(5)).toBe('27.46392');
			expect(createTerm('stdev(A2, A3, A4, A5, A6, A7, A8, A9, A10, A11)', sheet).value.toFixed(2)).toBe('27.46');
		});
		it('should ignore non numbers', () => {
			...
			expect(createTerm('stdev(A2:A11)', sheet).value.toFixed(2)).toBe('28.36');
			expect(createTerm('stdev(A2, A3, A4, A5, A6, A7, A8, A9, A10, A11)', sheet).value.toFixed(2)).toBe('28.36');
		});
		it(`should return  ${ERROR.DIV0} if only 1 or no value is available`, () => {
			...
			expect(createTerm('stdev()', sheet).value).toBe(ERROR.ARGS);
			expect(createTerm('stdev(B1)', sheet).value).toBe(ERROR.DIV0);
			expect(createTerm('stdev(B1:B11)', sheet).value).toBe(ERROR.DIV0);
			expect(createTerm('stdev(A2:A3)', sheet).value).toBe(ERROR.DIV0);
			expect(createTerm('stdev(A4)', sheet).value).toBe(ERROR.DIV0);
			expect(createTerm('stdev(A4:A4)', sheet).value).toBe(ERROR.DIV0);
			expect(createTerm('stdev(A2:A4)', sheet).value).toBe(ERROR.DIV0);
		});
		it('should return an error if values contain an error', () => {
			...
			expect(createTerm('stdev(A2:A5)', sheet).value).toBe(ERROR.VALUE);
			expect(createTerm('stdev(A5:A12)', sheet).value).toBe(ERROR.VALUE);
			expect(createTerm('stdev(A2:A12)', sheet).value).toBe(ERROR.VALUE);
			expect(createTerm('stdev(A2, A3, A4, A5)', sheet).value).toBe(ERROR.VALUE);
			expect(createTerm('stdev(A5, A6, A7, A8, A9, A10, A11)', sheet).value).toBe(ERROR.VALUE);
			expect(createTerm('stdev(A2, A3, A4, A5, A6, A7, A8, A9, A10, A11)', sheet).value).toBe(ERROR.VALUE);
		});
	});
});

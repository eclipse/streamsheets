...
const doIfNoError = (fn) => (values) => Error.isError(values) ? values : fn(values);

const runWith = (fn, sheet, terms) => fn(toNumbers(sheet, terms, onTerms.iterateAllTermsValues));

const avg = pipe(testValues(0, Error.code.DIV0), doIfNoError(calculate.avg));
...
const stdDev = pipe(testValues(1, Error.code.DIV0), doIfNoError(calculate.standardDerivation));
...
const correl = (sheet, ...terms) =>
	runFunction(sheet, terms)
		...
		.validate((arrayX, arrayY) => (arrayX.length === 0 || arrayY.length === 0 ? Error.code.DIV0 : undefined))
		.validate((arrayX, arrayY) => (arrayX.length !== arrayY.length ? Error.code.NA : undefined))
		.run((arrayX, arrayY) => {
...
const forecast = (sheet, ...terms) =>
	runFunction(sheet, terms)
		.withArgCount(3)
		.mapNextArg((xTerm) => (xTerm ? convert.toNumberStrict(xTerm.value, Error.code.VALUE) : Error.code.VALUE))
		...
		.validate((xVal, arrayY, arrayX) =>
			arrayX.length !== arrayY.length || arrayX.length === 0 || arrayY.length === 0 ? Error.code.NA : undefined
		)

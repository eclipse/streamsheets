...
describe('time.aggregate', () => {
	it.skip('should aggregate values of defined key by specified method', async () => {
		const machine = newMachine({ cycletime: 1000 });
		...
		createCellAt('A4', { formula: 'time.aggregate(A3, "v1", 2, 1/1000)' }, sheet);
		const cell = sheet.cellAt('A4');
		const term = cell.term;
		expect(cell.value).toBe(true);
		await machine.step();
		await waitFor(2);
		expect(term._store.size).toBe(1);
		expect(term._store.values('v1')).toEqual([2]);
		await machine.step();
		await waitFor(2);
		expect(term._store.values('v1')).toEqual([2, 3]);
		await machine.step();
		expect(term._store.values('v1')).toEqual([2, 3, 4]);
		// expect(term._timestore.size).toBe(3);
		// expect(term._timestore.values('v1')).toEqual([2, 3, 4]);
	});
	it('should return an error if first parameter does not reference time.store', () => {
		const machine = newMachine({ cycletime: 1000 });
		const sheet = machine.getStreamSheetByName('T1').sheet;
		createCellAt('A1', 'v1', sheet);
		createCellAt('A4', { formula: 'time.aggregate(A1, "v1")' }, sheet);
		const cell = sheet.cellAt('A4');
		// const term = cell.term;
		expect(cell.value).toBe(ERROR.VALUE);
	});
	it.skip('should store its values to cell info if no target specified', () => {

	});
	it.skip('should support period parameter', async () => {
		const machine = newMachine({ cycletime: 1000 });
		const sheet = machine.getStreamSheetByName('T1').sheet;
		createCellAt('A1', 'v1', sheet);
		createCellAt('B1', { formula: 'B1+1' }, sheet);
	...
	it.skip('should support timestamp parameter', async () => {
		const machine = newMachine({ cycletime: 1000 });
		const sheet = machine.getStreamSheetByName('T1').sheet;
		createCellAt('A1', 'v1', sheet);
		createCellAt('B1', { formula: 'B1+1' }, sheet);
	...
	it.skip('should sort entries by timestamp', async () => {
		const machine = newMachine({ cycletime: 1000 });
		const sheet = machine.getStreamSheetByName('T1').sheet;
		createCellAt('A1', 'v1', sheet);
		createCellAt('B1', { formula: 'B1+1' }, sheet);
	...
	it.skip('if timestamp is same last added comes after existing entry', async () => {
		const machine = newMachine({ cycletime: 1000 });
		const sheet = machine.getStreamSheetByName('T1').sheet;
		createCellAt('A1', 'v1', sheet);
		createCellAt('B1', { formula: 'B1+1' }, sheet);
	...
	it.skip('should be allowed to add values with undefined values!!', async () => {
		const machine = newMachine({ cycletime: 1000 });
		const sheet = machine.getStreamSheetByName('T1').sheet;
		createCellAt('A1', 'v1', sheet);
		createCellAt('B1', { formula: 'B1+1' }, sheet);
	...
	it.skip('should support multiple values', async () => {
		const machine = newMachine({ cycletime: 1000 });
		const sheet = machine.getStreamSheetByName('T1').sheet;
		createCellAt('A1', 'v1', sheet);
	...
	it.skip('should ignore values without a key', async () => {
		const machine = newMachine({ cycletime: 1000 });
		const sheet = machine.getStreamSheetByName('T1').sheet;
		createCellAt('A1', 'v1', sheet);
		createCellAt('A3', 'v3', sheet);
	...
	it.skip(`should return error ${ERROR.ARGS} if required parameter is missing`, () => {
	...
	it.skip(`should return ${ERROR.LIMIT} if limit is reached`, async () => {
		const machine = newMachine({ cycletime: 1000 });
		const sheet = machine.getStreamSheetByName('T1').sheet;
		createCellAt('A1', 'v1', sheet);
		createCellAt('B1', { formula: 'B1+1' }, sheet);

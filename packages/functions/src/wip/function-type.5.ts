...
function transformParams(params: SheetParam[]) {
	if (params.length === 3) {
		return transformParams3(<[SheetParam, SheetParam, SheetParam]>params);
	} else if (params.length === 4) {
		return transformParams4(<[SheetParam, SheetParam, SheetParam, SheetParam]>params);
	}
}

function transformParams1<S extends [SheetParam]>(params: S): SheetParamsResolver<S> {
	return <SheetParamsResolver<S>>params.map(transfromParam);
}

function transformParams2<S extends [SheetParam, SheetParam]>(params: S): SheetParamsResolver<S> {
	return <SheetParamsResolver<S>>params.map(transfromParam);
}

function transformParams3<S extends [SheetParam, SheetParam, SheetParam]>(params: S): SheetParamsResolver<S> {
	return <SheetParamsResolver<S>>params.map(transfromParam);
}
function transformParams4<S extends [SheetParam, SheetParam, SheetParam, SheetParam]>(
	params: S
): SheetParamsResolver<S> {
	return <SheetParamsResolver<S>>params.map(transfromParam);
}
function transformParams5<S extends [SheetParam, SheetParam, SheetParam, SheetParam, SheetParam]>(
	params: S
): SheetParamsResolver<S> {
	return <SheetParamsResolver<S>>params.map(transfromParam);
}
function transformParams6<S extends [SheetParam, SheetParam, SheetParam, SheetParam, SheetParam, SheetParam]>(
	params: S
): SheetParamsResolver<S> {
	return <SheetParamsResolver<S>>params.map(transfromParam);
}
function transformParams7<
	S extends [SheetParam, SheetParam, SheetParam, SheetParam, SheetParam, SheetParam, SheetParam]
>(params: S): SheetParamsResolver<S> {
	return <SheetParamsResolver<S>>params.map(transfromParam);
}
function transformParams8<
...
const aasdf: SheetParamsResolver<
	[
		{
			id: 'condition';
			name: {

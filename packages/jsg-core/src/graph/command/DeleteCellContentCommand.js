 ...
 ********************************************************************************/
 ...
 module.exports = class DeleteCellContentCommand extends AbstractItemCommand {
	 ...
	 combine(cmd) {
		 const cmdranges = cmd._ranges;
		 this._action = cmd._action;
		 cmdranges.forEach((cmdrange) => {
			 if (!this._ranges.find((range) => cmdrange.isEqualTo(range))) {
				 this._ranges.push(cmdrange);
			 }
		 });
		 return this;
	 }
	 ...
	 doAfterRedo(selection, viewer) {
		 this.selectRanges(viewer, this._ranges);
	 }

...
describe('subtree', () => {
	it('should extract json data from specified json element', () => {
		...
		expect(SUBTREE(sheet, createFuncTerm(sheet, 'inboxdata', createParamTerms('', '', 'Positionen', 3))).code)
	...
	it('should return #NO_MSG if extracting from an unknown message', () => {
		...
		expect(sheet.cellAt('A1').value.code).toBe(ERROR.NO_MSG);
		expect(sheet.cellAt('B1').value.code).toBe(ERROR.NO_MSG);
		expect(sheet.cellAt('C1').value.code).toBe(ERROR.NO_MSG);
		expect(sheet.cellAt('D1').value.code).toBe(ERROR.NO_MSG);
	});

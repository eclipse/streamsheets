const { Cell, Machine, Message, SheetIndex, StreamSheet, StreamSheetTrigger } = require('../..');
const { EXECUTE, GETEXECUTESTEP } = require('../../src/functions');
...
describe('execute', () => {
	it('should trigger execution from one streamsheet to another', () => {
	...
	it('should trigger execution as often as defined by repeat parameter', () => {
		const t1 = machine.getStreamSheetByName('T1');
		const t2 = machine.getStreamSheetByName('T2');
		const sheet1 = t1.sheet.load({
			cells: {
	...
	it('should restart repeated execution after machine stop', () => {
		const t1 = machine.getStreamSheetByName('T1');
		const t2 = machine.getStreamSheetByName('T2');
		const sheet1 = t1.sheet.load({
			cells: {
	...
	// DL-710:
	it('should go on with next loop element after resume from repeated execute', () => {
		const t1 = machine.getStreamSheetByName('T1');
		const t2 = machine.getStreamSheetByName('T2');
		t1.sheet.load({
			cells: {
	...
	it('should trigger execution in endless mode until return is called', () => {
		const t1 = machine.getStreamSheetByName('T1');
		const t2 = machine.getStreamSheetByName('T2');
		const sheet1 = t1.sheet.load({
			cells: {
	...
	it('should trigger execution as often as defined by repeat parameter and resume with return on each', () => {
		const t1 = machine.getStreamSheetByName('T1');
		const t2 = machine.getStreamSheetByName('T2');
		const sheet1 = t1.sheet.load({
			cells: {
	...
	it('should display return value after execution in endless mode', () => {
		const t1 = machine.getStreamSheetByName('T1');
		const t2 = machine.getStreamSheetByName('T2');
		const sheet1 = t1.sheet.load({
			cells: {
	...
	it('should not be triggered by machine step', () => {
		const t2 = machine.getStreamSheetByName('T2');
	...
	it('should not trigger execution if repeat counter is 0', () => {
		const t1 = machine.getStreamSheetByName('T1');
		const t2 = machine.getStreamSheetByName('T2');
		const sheet1 = t1.sheet.load({
			cells: {
	...
	it('should handle replacing execute term while running', () => {
		const t1 = machine.getStreamSheetByName('T1');
		const t2 = machine.getStreamSheetByName('T2');
	...
	// DL-1075
	it('should be possible to pass a complete message to triggered streamsheet', () => {
		// setup streamsheets:
		const t1 = machine.getStreamSheetByName('T1');
	...
	it('should pass message to triggered streamsheet only on next repeat step', () => {
		// setup streamsheets:
		const t1 = machine.getStreamSheetByName('T1');
		t1.trigger = StreamSheetTrigger.create({ type: 'once', repeat: 'endless' });
	...
	it('should handle message with loop element', () => {
		// setup streamsheets:
		const t1 = machine.getStreamSheetByName('T1');
		t1.trigger = StreamSheetTrigger.create({ type: 'once', repeat: 'endless' });
	...
	it('should read next loop element in endless mode only if sheet returns', () => {
		...
		machine.step(); // will RETURN here
		expect(t2.stats.executesteps).toBe(1);
		expect(t2.getCurrentLoopPath()).toBe('[Data][Step3]');
		expect(sheet2.cellAt('A4').value).toBe(1);
		expect(sheet2.cellAt('A5').value).toBe(true);
		expect(sheet2.cellAt('B3').value).toBe('Green');
		machine.step();	// NEXT REPEAT
		expect(t2.stats.executesteps).toBe(2);
		expect(t2.getCurrentLoopPath()).toBe('[Data][Step1]');
		expect(sheet2.cellAt('A4').value).toBe(2);
		expect(sheet2.cellAt('A5').value).toBe(false);
	...
	it('should read same loop element in endless mode from passed message until return', () => {
		// setup streamsheets:
		const t1 = machine.getStreamSheetByName('T1');
		t1.trigger = StreamSheetTrigger.create({ type: 'always' });
		t1.sheet.load({ cells: {
		...
		machine.step(); // will invoke RETURN() -> on next step we take next loop element
		expect(t2.inbox.messages.length).toBe(1);
		expect(t2.stats.executesteps).toBe(1);
		expect(t2.getCurrentLoopPath()).toBe('[Data][Step1]');
		expect(sheet2.cellAt('B3').value).toBe('Red');
...
describe('concatenated execute() usage', () => {
	// DL-1114
	it('should be possible to trigger a streamsheet which triggers another streamsheet', () => {
	...
	// DL-1663
	it('should not calculate twice if streamsheet is triggered by execute and endless mode in one step', () => {
		machine.removeAllStreamSheets();
		// setup 3 streamsheets: T1 executes T2 executes T3
...
describe('execute stream sheet which has own message stream', () => {
	it('should not add first received message twice on first machine step', () => {
	...
	it('should use next message if current one is processed', () => {
		machine.getStreamSheetByName('T1').updateSettings({ trigger: { type: 'execute', repeat: 'once' } });
		machine.getStreamSheetByName('T2').updateSettings({ trigger: { type: 'start', repeat: 'endless' } });
		machine.getStreamSheetByName('T2').sheet.load({ cells: { B4: { formula: 'execute("T1",1)' } } });
		// add messages:
	...
	it('should use next message if current one is processed with loop', () => {
		...
		machine.step(); // we pop current message from inbox:
		expect(machine.getStreamSheetByName('T1').inbox.size).toBe(2);

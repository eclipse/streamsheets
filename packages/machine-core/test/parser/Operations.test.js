...
const validate = (expr, sheet = SHEET) => {
	...
	return ({
		...
		error: (res) => {
			expect(term.value.code).toBe(res);
		},
		between: (min, max) => {
...
describe('Operations', () => {
	it('should use default value if one value for undefined left or right ', () => {
		...
		validate('"Hi" - (A3 - "Hi")').error(ERROR.VALUE);
	});
	...
	describe('should behave excel like for +, -, * and / operations', () => {
		...
		it('with string as left argument', () => {
			validate('A1+A1', sheet).error(ERROR.VALUE);
			validate('A1+B1', sheet).error(ERROR.VALUE);
			validate('A1+C1', sheet).error(ERROR.VALUE);
			validate('A1+D1', sheet).error(ERROR.VALUE);
			validate('A1+E1', sheet).error(ERROR.VALUE);
		...
		it('should convert an empty string to 0', () => {
			...
			validate('"" / 1', sheet).equals(0);
			validate('1 / ""', sheet).error(ERROR.DIV0);
			validate('"" / 0', sheet).error(ERROR.DIV0);
			validate('0 / ""', sheet).error(ERROR.DIV0);
		});
		it('with number as left argument', () => {
			validate('B1+A1', sheet).error(ERROR.VALUE);
			...
			validate('B1/C1', sheet).equals(23);
			validate('B1/D1', sheet).error(ERROR.DIV0);
			validate('B1/E1', sheet).error(ERROR.DIV0);
		});

		it('with true as left argument', () => {
			validate('C1+A1', sheet).error(ERROR.VALUE);
			...
			validate('C1/C1', sheet).equals(1);
			validate('C1/D1', sheet).error(ERROR.DIV0);
			validate('C1/E1', sheet).error(ERROR.DIV0);
		});

		it('with false as left argument', () => {
			validate('D1+A1', sheet).error(ERROR.VALUE);
			...
			validate('D1/B1', sheet).equals(0);
			validate('D1/C1', sheet).equals(0);
			validate('D1/D1', sheet).error(ERROR.DIV0);
			validate('D1/E1', sheet).error(ERROR.DIV0);
		});

		it('with null as left argument', () => {
			validate('E1+A1', sheet).error(ERROR.VALUE);
			...
			validate('E1/B1', sheet).equals(0);
			validate('E1/C1', sheet).equals(0);
			validate('E1/D1', sheet).error(ERROR.DIV0);
			validate('E1/E1', sheet).error(ERROR.DIV0);
		});
	});
	describe('unaray operator - should behave excel', () => {
		const sheet = new Sheet().loadCells({ A1: 'hello', B1: 23, C1: true, D1: false, E1: null });
		it(`should return ${ERROR.VALUE} if right term is a string`, () => {
			validate('-A1', sheet).error(ERROR.VALUE);
		});
	});
});

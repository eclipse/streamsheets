...
describe('minifs', () => {
	...
	it('should support unaligned criteria and value ranges if they are of same height & width', () => {
		...
		expect(createTerm('minifs(A2:A5, B3:C6, "a")', sheet).value.code).toBe(ERROR.VALUE);
	});
	...
	it(`should return ${ERROR.VALUE} if criteria ranges has different rows or columns then sum range`, () => {
		...
		expect(createTerm('minifs(A2:A4, C2:C5, ">0")', sheet).value.code).toBe(ERROR.VALUE);
		expect(createTerm('minifs(A2:B3, C2:D6, ">0")', sheet).value.code).toBe(ERROR.VALUE);
		expect(createTerm('minifs(A2:B3, C2:C2, ">0")', sheet).value.code).toBe(ERROR.VALUE);
	});
});

...
const putToTarget = (sheet, targetTerm, data) => {
	try {
		if (isInboxTerm(targetTerm) || isOutboxTerm(targetTerm)) {
			...
			const box = isInboxTerm(targetTerm) ? getInbox(sheet, path.shift()) : getOutbox(sheet);
			putToMessageBox(box, data, path);
		} else {
			putToMessageBox(getOutbox(sheet), data, ['']);
		}
	} catch (e) {
		putToMessageBox(getOutbox(sheet), data, ['']);
	}
...
const request = (s, ...t) =>
		...
		.withSheet()
		.withMachine()
		.with(({ messageTerm, machine }) => createMessageFromTerm(messageTerm, machine))
		.with(({ timeoutTerm }) => termAsNumber(timeoutTerm))
